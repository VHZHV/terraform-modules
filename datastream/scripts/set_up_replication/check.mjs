...
try {
  ...
  if (tablesWithoutPrivileges.length > 0) {
    console.log(
      `❌ Tables without SELECT privileges (${tablesWithoutPrivileges.length}): ${tablesWithoutPrivileges.join(", ")}`,
    );
  }

  if (tablesWithPrivileges.length > 0) {
    console.log(
      `✅ Tables with correct privileges (${tablesWithPrivileges.length}): ${tablesWithPrivileges.join(", ")}`,
    );
  }

  const allGood =
    missingTables.length === 0 && tablesWithoutPrivileges.length === 0;
