...
const permissionCommands = [
  ...
  { description: "Granting select to data", sql: `GRANT pg_read_all_data TO ${process.env.DATASTREAM_USERNAME};` }
];
...
console.log(`Executing: DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = '${process.env.PUBLICATION}') THEN CREATE PUBLICATION ${process.env.PUBLICATION} FOR ALL TABLES; END IF; END$$;`);
try {
...
console.log(`Executing: DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = '${process.env.SLOT}') THEN PERFORM PG_CREATE_LOGICAL_REPLICATION_SLOT('${process.env.SLOT}', 'pgoutput'); END IF; END$$;`);
try {
